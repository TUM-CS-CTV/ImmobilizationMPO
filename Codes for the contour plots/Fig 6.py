...
xlist = np.logspace(-6,-3, 1000)
ylist = np.logspace(-16,-13, 1000)
X, Y = np.meshgrid(xlist, ylist) 

# gives values to the non-varied parameters
D1         = 5.5*10**(-6)     # dm^2 min^-1
D2         = 5*10**(-6)       # dm^2 min^-1
kA         = 500              # dm^2 μmol^-1 min^-1
kB         = 500              # dm^2 μmol^-1 min^-1
L          = X                # dm 
A          = Y                # dm^2
N_alpha_A  = 5*10**(12) / 2   # -
N_alpha_B  = 5*10**(12) / 2   # -
...
ax.clabel(cp2, levels, inline=1, fmt='%1.1f', fontsize=20, manual= [(3*10**(-4),4*10**(-15)),(3*10**(-4),8*10**(-15)),(3*10**(-4),1.5*10**(-14)),(3*10**(-4),3*10**(-14))])
...
ax.set_xlabel('$L$ / (dm)', fontsize=20)
ax.set_ylabel('$A$ / (dm$^2$)', fontsize=20)
